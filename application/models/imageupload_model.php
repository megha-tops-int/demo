...
class imageupload_model extends CI_Model
{
    ...
    */
    ...
    function uploadBigImage($uploadFile,$bigImgPath,$smallImgPath,$thumb,$existImage,$fullImages,$sizeImgPath='')
    {	
    	//echo $fullImages;exit;
    	/*echo $thumbPath;
		echo $existImage;exit;*/
		if (!file_exists($bigImgPath)) {
			mkdir($bigImgPath, 0777);
		} 
		if (!empty($smallImgPath) && !file_exists($smallImgPath)) {
			mkdir($smallImgPath, 0777);
		} 
		...
		// if (!file_exists($sizeImgPath)) {
		// 	mkdir($sizeImgPath, 0777);
		// }
...
//		print_r($data);
		...
		$thumbPath2 = $this->config->item('movie_img_size');
		...
		{*/
			if(!empty($thumb) && $thumb=='thumb'){
				...
				else
				{
					...
					//$filename2 = $this->uploadSmallImage2($sourcePath,$thumbPath2,$fileName);						
				}
    ...
    */
	...
	public function uploadSmallImage2($sourceImgPath,$thumbPath)
    {
    	//echo $thumbPath;exit;
		$configThumb = array();  
		$configThumb['image_library'] = 'gd2';
		...
		$configThumb['maintain_ratio'] = FALSE;  
		$configThumb['width'] = 620;  
		$configThumb['height'] = 280;  
		$this->load->library('image_lib');
