...
<div style="font-family:Verdana, Geneva, sans-serif; font-size:12px; color:#333; line-height:25px; text-align:justify; margin:10px;">
                <p>Hello Admin,</p>
                <p>You have received message from <?= ucfirst($actdata['name']);?></p>
                <p>Contact details</p>
                <p>Name: <?=$actdata['name'];?></p>
                <p>Email: <?=$actdata['email'];?></p>
                <p>Contact number: <?=$actdata['contact_no'];?></p>
                <?php 
                if ($actdata['type'] == 1) 
                {
                    ?>
                    <p>Event title: <?=$actdata['title'];?></p>
                    <?php
                } 
                elseif ($actdata['type'] == 2) 
                {
                    ?>
                    <p>Project title: <?=$actdata['title'];?></p>
                    <?php
                }
                ?>
                <p>Message: <?=$actdata['message'];?></p>
