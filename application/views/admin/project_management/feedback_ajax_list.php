        ...
        ?>
            <tr <? if ($i % 2 == 1) { ?>class="bgtitle" <? } ?> >
                ...
                <td class="hidden-xs hidden-sm text-left">
                    ...
                    <button class="btn btn-xs btn-primary" title="<?php echo $this->lang->line('delete_record'); ?>"  onclick="delete_feedback('<?php echo $row['id'] ?>', '<?=!empty($row['first_name'])?rawurlencode(ucfirst(strtolower($row['first_name']))):'';?>', '<?= $path_comman_delete ?>', '<?php echo $project_id ?>');"> <i class="fa fa-times"></i> </button>
