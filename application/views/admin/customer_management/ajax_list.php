...
<th data-direction="desc" data-sortable="true" data-filterable="true" <?php if (isset($sortfield) && $sortfield == 'customer_name') {
    if ($sortby == 'asc') {
        echo "class = 'sorting_desc'";
    } else {
        echo "class = 'sorting_asc'";
    }
} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('customer_name', '<?php echo $sorttypepass; ?>', '<?= $path_comman ?>')">
    <?= $this->lang->line('common_label_name') ?>
    </a></th>
    
<th data-direction="desc" data-sortable="true" data-filterable="true" <?php if (isset($sortfield) && $sortfield == 'email') {
    if ($sortby == 'asc') {
        echo "class = 'sorting_desc'";
    } else {
        echo "class = 'sorting_asc'";
    ...
    foreach ($datalist as $row) {
        ?>
            <tr <? if ($i % 2 == 1) { ?>class="bgtitle" <? } ?> >
                ...
                <td class="hidden-xs hidden-sm "><?=!empty($row['customer_name'])?$row['customer_name']:'';?></td>
                ...
                <td class="hidden-xs hidden-sm text-left">
                    ...
                    <button class="btn btn-xs btn-primary" title="<?php echo $this->lang->line('delete_record'); ?>" onclick="deletepopup1('<?php echo $row['id'] ?>', '<?=!empty($row['customer_name'])?rawurlencode(ucfirst(strtolower($row['customer_name']))):'';?>', '<?= $path_comman ?>');"> <i class="fa fa-times"></i> </button>
