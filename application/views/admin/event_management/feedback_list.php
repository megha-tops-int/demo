      ...
      <div class="col-md-12">
        <div class="portlet">
          <div class="portlet-header">
            <h3> <i class="fa fa-calendar-check-o"></i>
              <?=$head_title_1?> <?php echo !empty($datalist[0]['title'])?'-'.$datalist[0]['title']:'';?>
            </h3>
          ...
          <div class="portlet-content">
            <!--<div class="row">-->
                    <div class="col-sm-12">
                       ...
                       <div class="col-sm-2">
                           <a class="btn btn-default howler" title="Details" href="<?= base_url('admin/' . $viewname . '/view_record') . '/' . $event_id; ?>">Details</a>
                       </div>
                       <div class="col-sm-6">
                           <a class="btn btn-primary howler" title="Feedback" href="<?= base_url('admin/' . $viewname . '/feedback') . '/' . $event_id; ?>">Feedback</a> 
                       </div>
...
<!--                      <a class="btn  pull-right btn-success howler" title="<?//=$this->lang->line('add_record')?>" href="<?//=base_url('admin/'.$viewname.'/add_record');?>">Add new <?//=  strtolower($sub_title);?>
                ...
                <div id="common_div">
                  <?=$this->load->view('admin/'.$viewname.'/feedback_ajax_list')?>
                </div>
              </div>
            </div>
            <!-- /.table-responsive --> 
...
<script>
	...
	$('body').on('click','#selecctall',function(e){
	 ...
	 $('#allcheck').click(function(){
		...
		if(val != '')
    	{
    		delete_all_feedback_multipal(val,'<?=$path_comman_delete?>','<?=$event_id?>');	
    	}
    	else
    	{
    		$.confirm({'title': 'Alert','message': " <strong> Please select atleast one oparation "+"<strong></strong>",'buttons': {'ok' : {'class'  : 'btn_center'}}});
    	}    
	});
       </script>
