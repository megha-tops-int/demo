            ...
            <div class="col-md-12">
                <div class="portlet">
                    <div class="portlet-header">
                        <h3> <i class="fa fa-calendar-check-o"></i><?= $is_edit . " " . $sub_title; ?></h3>
                        ...
                        <div class="col-sm-8">
                            <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname; ?>" id="<?php echo $viewname; ?>" method="post" accept-charset="utf-8" action="<?= $this->config->item('admin_base_url') ?><?php echo $path ?>" >
                                <div class="form-group">
                                    <label for="select-multi-input"><?= $this->lang->line('event_name') ?><span style="color:#F00">*</span></label>
                                    <input id="event_name" name="event_name" placeholder="Event name" class="form-control parsley-validated" type="text" data-required="required" value="<?= !empty($editRecord[0]['event_name']) ? htmlentities($editRecord[0]['event_name']) : ''; ?>" data-required="true">
                                </div>
                                
                                <div class="form-group">
                                    <label for="select-multi-input"><?php echo $this->lang->line('select_type');?><span style="color:#F00">*</span></label>
                                    <select name="event_type" id="event_type" class="form-control parsley-validated" data-required="required" data-required="true" >
                                        <option value=""> Select type </option>
                                        <option <?php if(!empty($editRecord[0]['event_type']) && $editRecord[0]['event_type'] == "Diversity") { echo "selected = selected"; } ?>  value="Diversity"> Diversity </option>
                                        <option <?php if(!empty($editRecord[0]['event_type']) && $editRecord[0]['event_type'] == "Leadership") { echo "selected = selected"; } ?>  value="Leadership"> Leadership </option>
                                        <option <?php if(!empty($editRecord[0]['event_type']) && $editRecord[0]['event_type'] == "Creativity") { echo "selected = selected"; } ?>  value="Creativity"> Creativity </option>
                                        <option <?php if(!empty($editRecord[0]['event_type']) && $editRecord[0]['event_type'] == "Wellbeing") { echo "selected = selected"; } ?>  value="Wellbeing"> Wellbeing </option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="event_date" readonly name="event_date"  placeholder="Date & time" class="form-control parsley-validated" type="text" data-required="required" value="<?php if (!empty($editRecord[0]['event_date']) && $editRecord[0]['event_date'] != '0000-00-00 00:00:00' && $editRecord[0]['event_date'] != '1970-01-01 00:00:00'){ echo date(COMMAN_DATE_FORMATE_JS, strtotime($editRecord[0]['event_date']));}?>">
                                    </div>
                                    <div class="col-sm-3">
                                        <label for="select-multi-input">From time<span style="color:#F00">*</span></label>
                                    <input id="from_time" readonly name="from_time"  placeholder="From time" class="form-control parsley-validated" type="text" data-required="required" value="<?php if (!empty($editRecord[0]['from_time']) && $editRecord[0]['from_time'] != '00:00:00'){ echo date(COMMAN_TIME_FORMATE_VIEW, strtotime($editRecord[0]['from_time']));}?>">
                                    </div>
                                    <div class="col-sm-3">
                                        <label for="select-multi-input">To time<span style="color:#F00">*</span></label>
                                    <input id="to_time" readonly name="to_time"  placeholder="To time" class="form-control parsley-validated" type="text" data-required="required" value="<?php if (!empty($editRecord[0]['to_time']) && $editRecord[0]['to_time'] != '00:00:00' && $editRecord[0]['to_time'] != '00:00:00'){ echo date(COMMAN_TIME_FORMATE_VIEW, strtotime($editRecord[0]['to_time']));}?>">
                                 </div>
                                 </div>  
                                
                                </div>
                                <div class="clear form-group add_image_div">
                                    ...
                                    <p> <span class="txt">&nbsp;</span>
                                      ...
                                      <?php } else { 
                                              //echo "em";
                                              if(!file_exists($this->config->item('event_img_small_path').$editRecord[0]['image'])){
                                                ?>
                                      ...
                                      <?php }
                                            ...
                                            { 
                                              ?>
                                      <img id="uploadPreview1" src="<?=$this->config->item('event_small_img_url').$editRecord[0]['image'] ?>"  width="100"  height="100" />
                                      <?php } }  ?>
                                      <?php } ?>
                                ...
                                <div class="form-group add_image_div">
                                    ...
                                    <div class="browse"> 
                                      ...
                                      <div class="browse_btn">
                                        <div class="file_input_div">
                                          <input type="button" value="Browse" class="file_input_button"  />
                                          <input type="file" alt="1" name="logo" id="logo" onchange="showimagepreview_logo(this)" class="file_input_hidden form-control parsley-validated"/>
                                        </div>
                                      </div>
                                      <input class="image_upload" type="hidden"  data-bvalidator="extension[jpg:png:jpeg:bmp:gif]" data-bvalidator-msg="Please upload jpg | jpeg | png | bmp | gif file only" name="hiddenFile" id="hiddenFile" value="" />
                                      <input type="hidden" name="oldfile_logo" id="oldfile_logo" value="<?=(!empty($editRecord[0]['logo'])?$editRecord[0]['logo']:'');?>" />
                                      <input type="hidden" name="oldfile1_logo" id="oldfile1_logo" value="<?=(!empty($editRecord[0]['logo'])?$editRecord[0]['logo']:'');?>" />
                                    </div>
                                    <p> <span class="txt">&nbsp;</span>
                                      <?php if(empty($editRecord)) { ?>
                                      <img id="uploadPreview1_logo" class="noimage" src="<?=base_url('images/no_image.jpg')?>"  width="100" height="100" style="display:none;"/>
                                      <?php } else { ?>
                                      <?php if(empty($editRecord[0]['logo'])) { ?>
                                      <img id="uploadPreview1_logo" src="<?=base_url('images/no_image.jpg')?>"  width="100"  height="100" />
                                      <?php } else { 
                                              //echo "em";
                                              if(!file_exists($this->config->item('event_logo_img_small_path').$editRecord[0]['logo'])){
                                                ?>
                                      <img id="uploadPreview1_logo" class="noimage" src="<?=base_url('images/no_image.jpg')?>"  width="100" />
                                      <?php }
                                            else
                                            { 
                                              ?>
                                      <img id="uploadPreview1_logo" src="<?=$this->config->item('event_logo_small_img_url').$editRecord[0]['logo'] ?>"  width="100"  height="100" />
                                      <?php } }  ?>
                                      <?php } ?>
                                      <span id="delete_btn_logo" style="<?php echo !empty($editRecord) ? empty($editRecord[0]['logo']) ? 'display:none' : '' : 'display:none' ?>"> <a title="Delete Image" class="btn btn-xs btn-primary mar_top_con_my delete_food_div_button1" onclick="return delete_image_logo('<?=!empty($editRecord[0]['id'])?$editRecord[0]['id']:''?>','<?=!empty($editRecord[0]['logo'])?$editRecord[0]['logo']:''?>');"> <i class="fa fa-times"></i> </a></span> </p>
                                    <label> To get best resolution preferred image size is 65 X 65. </label>
                                </div>
                                
                                <div class="form-group">
                                    <label for="select-multi-input">Organisers<span style="color:#F00">*</span></label>
                                     
                                     <textarea id="organizer" name="organizer" placeholder="Organisers" class="form-control parsley-validated" type="text" data-required="required"><?= !empty($editRecord[0]['organizer']) ? htmlentities($editRecord[0]['organizer']) : ''; ?></textarea>
                                ...
                                { 
                                    ...
                                    <div class="col-sm-2">
                                    <?php 
                                            if(!empty($fields_data[$i]['field_value']) && file_exists($this->config->item('event_fields_img_small_path').$fields_data[$i]['field_value'])) {
                                          ?>
                                    <img id='image_preview_<?=$i?>' title="Image" class="img-responsive" src="<?php echo $this->config->item('event_fields_small_img_url').$fields_data[$i]['field_value']?>"  style="width:80px !important;height:80px !important" />
                                    <?php }else{ ?>
                                ...
                                <div class="form-group addsavebtn">
                                    <input type="hidden" name="id" id="event_id" value="<?= !empty($editRecord[0]['id']) ? $editRecord[0]['id'] : ''; ?>" />
...
<script type="text/javascript">
    ...
    function setdefaultdata()
    {
        ...
        var old_file_logo = $("#oldfile1_logo").val();
        var file_logo = $("#logo").val();
        ...
        if (file_logo == '' && old_file_logo == '')
        {
        $.confirm({'title': 'Alert', 'message': " <strong> Please upload logo " + "<strong></strong>", 'buttons': {'ok': {'class': 'btn_center alert_ok'}}});
            return false;
        }
        $('#<?php echo $viewname ?>').parsley('validate');
...
//            {
                ...
                var filerdr = new FileReader();
                filerdr.onload = function(e) {
                $('#uploadPreview1_logo').attr('src', e.target.result);
                $('#uploadPreview1_logo').show();
                $('#delete_btn_logo').show();
                }
                filerdr.readAsDataURL(input.files[0]);
//            } else
//            {
...
//            }
          ...
          $.confirm({
              ...
              'buttons': {
                'Ok': {'class': 'btn_center alert_ok',  
                    'action': function(){
                      $("#logo").val('');
                      $("#uploadPreview1_logo").attr("src", "<?=base_url('images/no_image.jpg')?>");      
                      }}
               }
          });
          return false;
    ...
    $("#logo").change(function(e) {
        ...
        if ((file = this.files[0])) {
            ...
            img.onload = function() {
                ...
                {
                    $.confirm({'title': 'Alert', 'message': " <strong>  Invalid image size. Please upload 65 X 65 resolution image.</strong>", 'buttons': {'ok': {'class': 'btn_center alert_ok'}}});
                    $("#logo").val('');
                    $('#delete_btn_logo').hide();
                    $('#uploadPreview1_logo').attr('src', "<?php echo base_url() ?>images/no_image.jpg");
                    $('#uploadPreview1_logo').hide();
                    $("#oldfile1_logo").val('');
                    //location.reload();
                    return false;
    ...
    function delete_image_logo(id,image)
    { 
      if(id != '' && image != '')
      {
        $.confirm({
        'title': 'DELETE','message': " <strong> Are you sure want to delete logo?",'buttons': {'Yes': {'class': 'special',
        'action': function(){
            // $.ajax({
            //     type  : "POST",
            //     url   : "<?php echo $this->config->item('admin_base_url').$viewname.'/delete_profile_image';?>",
...
$(function () {
 ...
 $('#from_time').datetimepicker({
            locale: 'en',
            datepicker:false,
            format: '<?=COMMAN_TIME_FORMATE_VIEW?>',
 ...
 $('#to_time').datetimepicker({
            ...
            onShow: function() {
                this.setOptions({
                minTime: $("#from_time").val() ? $("#from_time").val() : false,
                format: '<?= COMMAN_TIME_FORMATE_VIEW ?>'
                })
            }
        });
