...
<th width="35%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if (isset($sortfield) && $sortfield == 'partner_name') {
    if ($sortby == 'asc') {
        echo "class = 'sorting_desc'";
    } else {
        echo "class = 'sorting_asc'";
    }
} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('partner_name', '<?php echo $sorttypepass; ?>', '<?= $path_comman ?>')">
    <?= $this->lang->line('partner_name') ?>
    </a></th>
    
<th width="42%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if (isset($sortfield) && $sortfield == 'website') {
    if ($sortby == 'asc') {
        echo "class = 'sorting_desc'";
    } else {
        echo "class = 'sorting_asc'";
    }
} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('website', '<?php echo $sorttypepass; ?>', '<?= $path_comman ?>')">
    ...
    foreach ($datalist as $row) {
        ?>
            <tr <? if ($i % 2 == 1) { ?>class="bgtitle" <? } ?> >
                ...
                <td class="hidden-xs hidden-sm "><?=!empty($row['partner_name'])?$row['partner_name']:'';?></td>
                <td class="hidden-xs hidden-sm "><?=!empty($row['website'])?$row['website']:'';?></td>
                <td align="center" class="hidden-xs hidden-sm ">
                  <img src='<?= !empty($row['logo'])?file_exists($this->config->item('partners_img_small_path').$row['logo'])?$this->config->item('partners_small_img_url').$row['logo']:base_url('images/no_image.jpg'):base_url('images/no_image.jpg')?>' width='50' height='50'>
                </td>
